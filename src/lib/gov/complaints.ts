...
function complaintSelect() {
  return db
    ...
    .from(schema.report)
    /*
     * Assignment hangs off the ISSUE now, so the path from a report to its
     * government work runs through the grouping link. A report that has not
     * been grouped yet simply has no assignment, which the left joins express.
     */
    .leftJoin(
      collabSchema.issueReport,
      eq(collabSchema.issueReport.reportId, schema.report.id),
    )
    .leftJoin(
      govSchema.complaintAssignment,
      eq(govSchema.complaintAssignment.issueId, collabSchema.issueReport.issueId),
    )
...
> {
  const [row] = await db
    ...
    .innerJoin(
      collabSchema.issueReport,
      eq(collabSchema.issueReport.issueId, govSchema.complaintAssignment.issueId),
    )
    .where(eq(collabSchema.issueReport.reportId, reportId))
    .limit(1);
...
export async function routeComplaint(input: {
  ...
  /*
   * Routing acts on the ISSUE this report was grouped into. A report that the
   * intake pipeline has not grouped yet has nothing to route — treated as
   * "not ready", the same answer as an unconfirmed report, because from the
   * officer's point of view it is the same situation: not yet actionable.
   */
  const [grouped] = await db
    .select({ issueId: collabSchema.issueReport.issueId })
    .from(collabSchema.issueReport)
    .where(eq(collabSchema.issueReport.reportId, input.reportId))
    .limit(1);

  if (!grouped) return { error: "not_ready" };
...
export async function rateComplaint(input: {
  ...
  const [resolved] = await db
    ...
    .where(
      and(
        eq(collabSchema.issueReport.reportId, input.reportId),
